...
pipeline {
    agent any
    environment {
        NEXUS_CREDENTIALS=credentials('nexus_lab4')
    }
    stages {
        ...
        stage("Step 6: Download from Nexus"){
            steps {
                script{
                    sh 'curl -X GET -u ${NEXUS_CREDENTIALS} "http://nexus:8081/repository/Repo_Lab_mod4/com/devopsusach2020/DevOpsUsach2020/0.0.1/DevOpsUsach2020-0.0.1.jar" -O'
                }
            }
        }
